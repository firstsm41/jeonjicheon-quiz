...
빔프로젝터·큰 화면용 페이지가 따로 있습니다. 학생 폰에는 노출되지 않고,
진행자가 이 주소를 열어 강당 화면에 띄웁니다.

**주소** — `.../present.html`
(배포 주소: https://firstsm41.github.io/jeonjicheon-quiz/present.html )

- 한 번에 한 문항씩 큰 글씨로 보여줍니다
- **방향키(← →)**, **화면 좌우 클릭**, 또는 하단 화살표로 넘깁니다
- 객관식은 선택지별 인원과 정답률, 주관식은 답을 묶어 많이 나온 순으로 표시
- 새 응답이 들어오면 실시간으로 갱신됩니다

진행 순서 예시: 학생들이 각자 폰으로 QR 접속 → 문제 풀이 →
진행자가 강당 화면에서 `present.html` 을 열어 문항별로 결과를 함께 확인.

## 진행자 모드

**"퀴즈 시작하기" 버튼을 0.8초간 길게 누르면** 비밀번호 창이 뜹니다.
기본 비밀번호는 **`2345`** 이며 `app.js` 의 `ADMIN_PIN` 에서 바꿀 수 있습니다.

진입하면 화면 위에 진행자 바가 나타나고, **참여자에게 결과 공개** 토글로
전체 결과 화면의 노출 여부를 제어합니다. 토글을 바꾸면 접속 중인 모든
참여자 화면에 실시간으로 반영됩니다.

참여자에게 '결과 보기'가 보이는 조건은 **두 가지를 모두** 만족할 때입니다.

1. 그 사람이 퀴즈를 끝까지 풀고 제출했을 것
2. 진행자가 결과 공개 토글을 켰을 것

진행자 본인은 조건과 무관하게 언제든 결과를 볼 수 있습니다.
진행자 모드는 그 기기에만 저장되며, 바의 **나가기** 로 해제합니다.

> 비밀번호는 브라우저에서 실행되는 코드에 들어 있어, 마음먹고 찾아보면 알 수
> 있습니다. 강의 진행 편의를 위한 장치이지 보안 장치는 아닙니다.
...
2. 왼쪽 메뉴 **SQL Editor** → `schema.sql` 내용을 붙여넣고 **Run**
   (이미 예전 버전을 실행한 프로젝트라면 `schema-settings.sql` 만 추가로 실행)
...
// 주관식
{
  type: 'text',
  q: '질문 내용 (주관식)',
  answer: 'CMI',              // 화면에 보여줄 정답
  accept: ['CMI', '씨엠아이'], // 정답으로 함께 묶을 표기들 (대소문자·공백 무시)
