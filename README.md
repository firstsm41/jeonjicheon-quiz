...
QR로 접속해 5문항을 풀고, **실시간으로 누적되는 전체 통계**를 함께 보는 웹앱입니다.
정적 사이트 + Supabase 조합이라 서버 관리가 필요 없습니다.
...
문항 개수를 바꿔도 점수 만점·분포 그래프가 자동으로 따라갑니다.
단, 문항을 바꿨다면 `QUIZ_SESSION` 값도 함께 바꿔야 이전 통계와 섞이지 않습니다.
